Puzzle.prototype = {
    ...
    initData(options){ //数据初始化赋值
        ...
        this.beforeHandle = options.beforeHandle || function(){return};
        ...
        setTimeout(function(){
            // self.oBlockMap=document.querySelectorAll('.block');
    ...
    checkWin(){ //将标准数组中的top 和 left 与  当前的backgroundPosition 的x y 进行对比
        ...
        for (var i = 0; i < this.oBlockMap.length; i++) {           
            ...
            var oBlockMap = this.oBlockMap[i]
            //top left  //当background-position 与 position坐标相同 则为赢
            var blockLeft = parseInt('-'+oBlockMap.style.left);
            var blockTop = parseInt('-'+oBlockMap.style.top);
            //  x y
            var imgLeft = parseInt(oBlockMap.style.backgroundPositionX);
            var imgTop = parseInt(oBlockMap.style.backgroundPositionY);
