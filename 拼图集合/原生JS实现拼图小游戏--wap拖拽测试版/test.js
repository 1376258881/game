...
var puzzleGame = function(param){
            ...
            this.btnStart.bind('touchstart',function(){
                ...
                if(self.hasStart == 0){//不在游戏中
                    ...
                    self.imgCells.css({
                        ...
                        var cell_mouse_x = e.originalEvent.touches[0].pageX - self.imgCells.eq(cellIndex_1).offset().left;
                        var cell_mouse_y = e.originalEvent.touches[0].pageY - self.imgCells.eq(cellIndex_1).offset().top;
    
                        $(document).bind('touchmove',function(e2){
                            self.imgCells.eq(cellIndex_1).css({
                                'z-index':'40',
                                'left':(e2.originalEvent.touches[0].pageX - cell_mouse_x - self.imgArea.offset().left) + 'px',
                                'top':(e2.originalEvent.touches[0].pageY - cell_mouse_y - self.imgArea.offset().top) + 'px'
                            });
    ...
    /* 加入图片，运行代码 */
    $(function(){
        var pg = new puzzleGame({'img':'1.png'})
    });
